...
# Utilizado para fazer a condição "ou"
...
class ProductManager(models.Manager):
    ...
    def expensive_products(self):
        return self.get_queryset().filter(price__gte=60)

    def cheap_toys(self):
        return self.get_queryset().filter(
            category__name='Category 2020', price__lte=60)

    def toys_or_expensive_items(self):
        query_filter = Q(category__name='Category 2020') | Q(price__gte=60)
